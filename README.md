...
Если покупатель на Plati.Market/Digiseller оплатил товар, но не прислал уникальный код, панель может сама написать ему в чат заказа и попросить отправить 16-значный код. По умолчанию сообщение уходит через 15 минут после оплаты. Задержка задаётся в `Настройки` -> `Задержка запроса кода, мин`, а текст меняется в `Шаблоны` -> `Digiseller` -> `Покупатель не прислал уникальный код`.
