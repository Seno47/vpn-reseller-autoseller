...
В настройках есть функция `Вечный онлайн Plati.Market`. Она поддерживает активность продавца через безопасные запросы к API переписок Digiseller. Для работы должны быть заполнены `Digiseller seller ID` и `Digiseller API key`; сообщения покупателям эта функция не отправляет.
...
- `Вечный онлайн Plati.Market` можно включить после установки в настройках маркетплейсов
