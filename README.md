...
Для Digiseller основной режим - URL-уведомления о продажах и сообщениях. Медленный polling можно оставить как запасной вариант настройкой `Fallback polling Digiseller`.
...
На Linux установщик добавляет отдельный host-updater. Панель сама не получает root-доступ и не управляет Docker напрямую: она только создаёт файл-запрос в `data/update-request.json`, а системный updater на сервере скачивает новую версию, делает бэкап кода, обновляет зависимости и перезапускает сервис или контейнер.
...
Обновить проект вручную, если кнопка обновления недоступна:
