...
| GGsel | GGsel присылает URL-уведомление о новом заказе, панель проверяет заказ через Seller API и отправляет выдачу в чат заказа. |
...
Для GGsel основной режим также URL-уведомления. Резервная проверка последних продаж запускается с настраиваемым интервалом, по умолчанию раз в 5 минут. Это снижает нагрузку на API и не даёт потерять заказ при задержке callback.

Если покупатель на Plati.Market/Digiseller оплатил товар, но не прислал уникальный код, панель может сама написать ему в чат заказа и попросить отправить 16-значный код. По умолчанию сообщение уходит сразу после уведомления об оплате. При необходимости пауза от 0 до 1440 минут задаётся в `Настройки` -> `Задержка перед запросом кода, мин`, а текст меняется в `Шаблоны` -> `Digiseller` -> `Покупатель не прислал уникальный код`.
...
- `URL-уведомления GGsel о заказах` - оставьте включённым;
- `Интервал fallback-проверки продаж GGsel` - обычно достаточно 300 секунд;
...
- Для GGsel скопируйте URL из карточки `URL-уведомления GGsel` и укажите его в notification settings товара, метод `POST`.
...
### URL-уведомления GGsel

1. Задайте публичный HTTPS `Base URL` в настройках панели.
2. Скопируйте готовую ссылку из карточки `URL-уведомления GGsel`.
3. В notification settings товара GGsel укажите тип `URL`, метод `POST`, вставьте ссылку и включите уведомление.
4. Не публикуйте этот URL: в нём есть секретная часть.

После callback панель не доверяет входящему телу запроса и повторно читает заказ через Seller API. Если callback задержится, fallback-проверка обнаружит продажу позже.
...
3. Укажите URL нового заказа из карточки `URL-уведомления GGsel` в notification settings товара.
4. Добавьте маппинг нужного лота и кнопки.
5. Сделайте тестовую покупку.
6. Проверьте, что callback появился в истории событий и панель отправила подписку в чат GGsel.
